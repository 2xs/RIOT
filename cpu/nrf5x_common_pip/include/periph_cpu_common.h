...
#define CPUID_ADDR          (PIP_NRF_FICR_FICR_DEVICEID_0)
#else
#define CPUID_ADDR          (PIP_NRF_FICR_FICR_DEVICEID_0)
#endif
/**
...
typedef struct {
#ifdef UARTE_PRESENT
    uint32_t dev;           /**< UART with EasyDMA device base
                             * register address */
#else
    uint32_t dev;           /**< UART device base register address */
#endif
...
typedef struct {
    uint32_t dev;                       /**< PWM device descriptor */
 ...
 */
#define SPI_SCKSEL          (dev(bus) + PIP_NRF_SPIM_SPIM_PSEL_SCK_INDEX)    /**< Macro for SPI clk */
#define SPI_MOSISEL         (dev(bus) + PIP_NRF_SPIM_SPIM_PSEL_MOSI_INDEX)   /**< Macro for SPI mosi */
#define SPI_MISOSEL         (dev(bus) + PIP_NRF_SPIM_SPIM_PSEL_MISO_INDEX)   /**< Macro for SPI miso */
 ...
 */
void shared_irq_register_spi(uint32_t bus,
 ...
 * @param   arg Argument to pass to the handler
 */
void shared_irq_register_i2c(uint32_t bus,
                              shared_irq_cb_t cb, void *arg);
 ...
 * @param   arg Argument to pass to the handler
 */
void shared_irq_register_uart(uint32_t bus,
                              shared_irq_cb_t cb, void *arg);
 ...
 */
void nrf5x_i2c_acquire(uint32_t bus, shared_irq_cb_t cb, void *arg);
 ...
 */
void nrf5x_i2c_release(uint32_t bus);
 ...
 * @param   arg ISR handler argument
 */
void nrf5x_spi_acquire(uint32_t bus, shared_irq_cb_t cb, void *arg);
 ...
 * @param   bus bus to release exclusive access on
 */
void nrf5x_spi_release(uint32_t bus);

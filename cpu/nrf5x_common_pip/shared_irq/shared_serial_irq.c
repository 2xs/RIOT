...
#ifdef NRF_SPIM4_S
    else if (dev == PIP_NRF_SPIM_SPIM4_BASE) {
        return 4;
    }
#endif
    else {
...
static inline size_t _uart_dev2num(uint32_t dev)
{
...
void nrf5x_i2c_acquire(uint32_t bus,
                       shared_irq_cb_t cb, void *arg)
{
    size_t num = _i2c_dev2num(bus);
...
void nrf5x_spi_acquire(uint32_t bus,
                       shared_irq_cb_t cb, void *arg)
{
    size_t num = _spi_dev2num(bus);
    mutex_lock(&_locks[num]);
